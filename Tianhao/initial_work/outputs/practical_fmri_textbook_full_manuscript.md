...
**Figure 1.1 (planned).** Net magnetization and signal strength dependence on magnetic field strength, B0. Adapt from source slides 2-3.
...
**Figure 2.1 (planned).** Rotating-frame view before excitation and the RF pulse. Adapt from source slides 4-5.
...
**Figure 2.2 (planned).** RF-pulse duration and flip-angle setting. Adapt from source slide 6.
...
**Figure 2.3 (planned).** Magnetization after excitation and early dephasing. Adapt from source slides 7-8.
...
**Figure 2.4 (planned).** Signal detection and the NMR signal. Adapt from source slides 9-10.
...
**Figure 3.1 (planned).** The spin echo overview. Adapt from source slide 11.
...
**Figure 3.2 (planned).** Spin-echo excitation and first evolution. Adapt from source slides 12-13.

**Figure 3.3 (planned).** Spin-echo refocusing and after-refocusing stages. Adapt from source slides 14-15.

**Figure 3.4 (planned).** Spin echo after second evolution. Adapt from source slide 16.
...
**Figure 3.5 (planned).** Longitudinal relaxation time (T1). Adapt from source slide 17.
...
**Figure 3.6 (planned).** Molecular origins of relaxation and the concept of spin temperature in relaxation. Adapt from source slides 18-19.
...
**Figure 3.7 (planned).** Diffusion, T2 and T2*. Adapt from source slide 21.
...
**Figure 3.8 (planned).** Chemical shift. Adapt from source slide 22.
...
**Figure 4.1 (planned).** Main MRI components. Adapt from source slide 25.
...
**Figure 4.2 (planned).** Gradient coil. Adapt from source slide 27.
...
**Figure 4.3 (planned).** Receive coil arrays and 32-channel coil. Adapt from source slides 28-29.
...
**Figure 4.4 (planned).** Receive field heterogeneity. Adapt from source slide 30.
...
**Figure 5.1 (planned).** Conjugate-variable relationships between time and frequency, and between space and k-space. Adapt from source slides 32–3.
...
**Figure 5.2 (planned).** Complex-wave decomposition into component frequencies under the Fourier transform. Adapt from source slide 34.

**Figure 5.3 (planned).** Fourier decomposition example showing component-frequency peaks for a complex signal. Adapt from source slide 35.

**Figure 5.4 (planned).** Fourier transform notation, Euler relation, and example frequency spectra from time-varying signals. Adapt from source slide 36.

**Figure 5.5 (planned).** Time-domain and frequency-domain illustration of Fourier decomposition for a complex signal. Adapt from source slide 37.

**Figure 5.6 (planned).** Useful Fourier pairs that recur in MRI intuition. Adapt from source slide 38.
...
**Figure 5.7 (planned).** One-dimensional gradient encoding and how position maps to frequency components. Adapt from source slides 39–2.
...
**Figure 5.8 (planned).** Historical projection imaging and Lauterbur’s early MRI logic. Adapt from source slide 43.
...
**Figure 6.1 (planned).** Sinc-shaped RF pulse and rectangular excitation bandwidth, showing frequency-selective slice selection under \(G_z\). Adapt from source slides 44–7.
...
**Figure 6.3 (planned).** Three-stage gradient-echo formation sequence. Adapt from source slides 49–3.
...
**Figure 7.1 (planned).** Image space versus k-space, showing that they are Fourier pairs rather than different anatomical spaces. Adapt from source slides 54–6.
...
**Figure 7.2 (planned).** Gradient-echo trajectory through kx and phase-encoding shifts through ky. Adapt from source slides 61–8.
...
**Figure 7.3 (planned).** Building a 2D k-space matrix line by line. Adapt from source slides 69–7.
...
**Figure 7.4 (planned).** Effects of preserving only low or high spatial frequencies; reduced resolution and edge-enhanced images. Adapt from source slides 78–1.
...
**Figure 7.5 (planned).** Aliasing and truncation artifact examples. Adapt from source slides 83–4.
...
**Figure 8.2 (planned).** Real EPI pulse sequence with slice select, echo train, and correction components. Adapt from source slides 114–16.
...
**Figure 8.3 (planned).** Example of good EPI and corresponding temporal SNR / standard-deviation images. Adapt from source slides 117–18.
...
**Figure 9.1 (planned).** Nyquist ghosting mechanism: odd/even line mismatch, zigzag k-space, and half-FOV ghost formation. Adapt from source slides 89–2.
...
**Figure 9.2 (planned).** Time-varying ghosting shown with standard-deviation imagery. Adapt from source slides 93–5.
...
**Figure 9.3 (planned).** Fat suppression on/off and chemical-shift contamination in EPI. Adapt from source slides 97–9.
...
**Figure 9.4 (planned).** Distortion and bandwidth comparison between read and phase directions. Adapt from source slides 102–06.
...
**Figure 9.5 (planned).** Signal dropout and slice-thickness dependence. Adapt from source slides 107–10.
...
**Figure 10.1 (planned).** Susceptibility gradients near air-bone-brain interfaces, with phase-map illustration. Adapt from source slides 121–23.
...
**Figure 10.3 (planned).** Flip-angle / inflow effects on response magnitude and timing. Adapt from source slides 126–30.
...
**Figure 10.4 (planned).** Receive-bias-motion interaction before and after perfect motion correction under homogeneous versus heterogeneous receive fields. Adapt from source slides 131–39.
...
**Figure 11.1 (planned).** Conjugate symmetry and partial Fourier omission of early versus late echoes. Adapt from source slides 141–58.
...
**Figure 11.2 (planned).** GRAPPA R = 2 trajectory, ACS logic, and motion mismatch scenarios. Adapt from source slides 159–67.
...
**Figure 11.3 (planned).** SMS acquisition and coil-based slice unaliasing. Adapt from source slides 169–77.
...
**Figure 11.4 (planned).** Multi-echo TE dependence for BOLD versus non-BOLD signal components. Adapt from source slides 178–82.
...
**Figure 12.1 (planned).** Good EPI reference images and normal ghosting examples. Adapt from source slides 188–89 and 198.
...
**Figure 12.2 (planned).** Scalp ghosts and eye-movement ghost placement. Adapt from source slides 190–91.
...
**Figure 12.4 (planned).** Residual aliasing examples for GRAPPA, SMS, and SMS + GRAPPA. Adapt from source slides 194–97.
...
**Figure 12.5 (planned).** Movement and hardware failure atlas: eyes, head, talking, feet, coil instability, metal, RF interference, gradient spikes, RF coil spikes. Adapt from source slides 200–14.
...
**Figure 12.6 (planned).** Troubleshooting decision checklist. Adapt from source slides 216–17.
...
**Figure 13.1 (planned).** Concept map of biological confound mechanisms and how they interact with acquisition and interpretation. Adapt from source slides 219–24.
...
**Figure 13.3 (planned).** Decision chart for MRI-based versus auxiliary-data-based confound assessment. Adapt from source slides 225–26.
